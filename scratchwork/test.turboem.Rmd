...
```{r}
  library(turboEM)
  res = turboem(par=pi.init, control.run=list(convtype = "objfn",tol=1e-5),fixptfn=fixpoint, objfn=penloglik, pconstr=pconstr,method=c("em", "squarem", "pem","decme","qn"), matrix_lik=lik,prior=prior)
  options(digits=13)
  res
  library(devtools)
...
```{r}
  ...
  prior=c(1,1,1)
  pi.init=c(0.3,0.2,0.5)
  res = turboem(par=pi.init, control.run=list(convtype = "objfn",tol=1e-5),fixptfn=fixpoint, objfn=penloglik, pconstr=pconstr,method=c("em", "squarem"), matrix_lik=lik,prior=prior)
  options(digits=13)
  res
```
...
```{r}
  library(SQUAREM)
  system.time(res3<-squarem(par=pi.init,fixptfn=fixpoint, objfn=penloglik,matrix_lik=lik, prior=prior))
```
