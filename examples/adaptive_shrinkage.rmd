...
```{r plotting}
df = data.frame(betahat = c(sim.spiky$input$betahat,sim.bignormal$input$betahat),
                beta_est = c(beta.spiky.ash$PosteriorMean, beta.bignormal.ash$PosteriorMean),
                lfsr = c(beta.spiky.ash$lfsr, beta.bignormal.ash$lfsr),
                s=c(s,s),
                scenario=c(rep("spiky",NSAMP),rep("big-normal",NSAMP)))
df$signif = df$lfsr<0.05
library(ggplot2)
library(scales)
ggplot(df,aes(x=betahat,y=beta_est,color=1/s)) + 
  xlab("Observed beta-hat") +
  ylab("Shrunken beta estimate") + geom_point() +  facet_grid(.~scenario) + geom_abline(intercept=0,slope=1) +
scale_colour_gradient2(midpoint = median(1/s),low="blue", mid="white",high="red")  +coord_fixed(ratio=1)
ggsave("ash.pdf")
```
