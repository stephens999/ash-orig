...
- For example: [https://github.com/jdblischak/singleCellSeq](https://github.com/jdblischak/singleCellSeq)
- A template (in progress): [https://github.com/jhsiao999/ashlar](https://github.com/jhsiao999/ashlar)
...
<!-- A quick ad for a recently-started project I call "Dynamic Statistical Comparisons": -->

<!-- - Overall Goal: Improve how the statistics and genomics (and other) communities make comparisons among methods. -->
...
We take a Bayesian perspective and use the following definitions
(see Storey, 2003, for connections with Frequentist definitions):
...
- Estimating $g$ comes down to estimating $\pi$. Joint estimation of $\pi_0,\pi$ easy by maximum likelihood (convex optimization).

- Setting $K$ large, with dense grid of $\sigma_k$, yields flexible unimodal symmetric distribution.
...
- The idea of inducing sparsity/shrinkage via unimodal distributions is very general!

- smash: Smoothing via adaptive shrinkage (wavelet-based)

- vash: shrinkage of variances and not just means. (e.g. ``moderated $t$ test", Smyth et al) [Mengyin Lu]

- flash: shrinkage of factors and loadings in factor analysis [Wei Wang; David Gerard]

- mash: allow for matrix of observations/effects $\hat\beta_{ij}$ [Sarah Urbut, Gao Wang]

- cash: allow $g(\cdot;\pi)$ to depend on covariates $X$ [Scott Powers, Lei Sun]
...
- Including  Chaoxing Dai, Mengyin Lu, Ester Pantaleo, Sen Tian, Wei Wang, Zhengrong Xing, Sarah Urbut
