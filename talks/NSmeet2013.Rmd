...
```{r, echo=FALSE,fig.height=3.5,fig.width=5,fig.cap=""}
...
```{r,echo=FALSE, fig.height=4,fig.cap=""}
plot(qval$qval[xx>1000],qval.high$qval,xlab="q values from all gene analysis", ylab="q values from high-count gene analysis", main="q values for high count genes",xlim=c(0,.5),ylim=c(0,.5))
...
```{r, echo=FALSE,fig.height=4,fig.cap=""}
h=hist(hh.betahat,freq=FALSE,main="BRCA, effect sizes",ylim=c(0,2),nclass=nc)
x = seq(-3,3,length=100)
lines(x,density(hh.ash,x),col=2,lwd=2)
lines(x,density(hh.ash.hu,x),col=3,lwd=2)
```
...
```{r, echo=FALSE, includ=FALSE}
  hh.ash.fdr = ash(hh.betahat,hh.sebetahat,usePointMass = TRUE,prior="nullbiased",VB=TRUE)
  hh.ash.shrink = ash(hh.betahat,hh.sebetahat)
```
...
```{r, echo=FALSE,fig.height=3,fig.width=3,fig.cap=""}
plot(zdat.ash$qvalue[xx>1000],zdat.ash.high$qvalue,xlab="q values from all data",ylab="q values from high only")
abline(a=0,b=1,col=2,lwd=2)
```
