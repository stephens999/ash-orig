---
title: "Compare Betahat vs Zscore Analysis"
...
simply replaces betahat with the standardized betahat, $\hat\beta_j$, and the standard errors
...
```{r}
  library(ashr)
  ash1.res = ash(betahat, betahat.se,method="fdr",model="EE")
```
...
```{r}
  ash2.res = ash(betahat, betahat.se,method="fdr", model="ES")
```
...
```{r}
  loglikEE=ashr:::loglik.ash(ash1.res, betahat, betahat.se,df=NULL, model="EE") 
  loglikES=ashr:::loglik.ash(ash2.res,betahat, betahat.se, df=NULL, model="ES")
  loglikEE-loglikES
```

Then the log likelihood ratio is loglikEE-loglikES=`r loglikEE-loglikES`. This highly negative
...
Finally, note that the likelihood should always be computed under the same
model as was used when applying ash. You'll get a warning if you do this wrong, although it will still run:
```{r}
  ashr:::loglik.ash(ash2.res,betahat, betahat.se, df=NULL,model = "EE")
```
