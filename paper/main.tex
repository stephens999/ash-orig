...
\end{equation}
where $\delta_0(\cdot)$ denotes a point mass at 0, $\pi=(\pi_0,\dots,\pi_K)$ are mixture proportions to be estimated (with $\pi_k \geq 0, \sum_k \pi_k=1$), and $f_k$ are pre-specified 
...
caused by this behaviour. To quantify this, we ran \ashr 10 times on each of XX simulated datasets using a random initialization for $\pi$,
...
When using a mixture of normals (ash.n) the results were extremely stable: 95\% showed a negligible log-likelihood difference (< 0.02), 
and the largest difference was 0.8. When using mixtures of uniforms (ash.u,ash.hu) results were slightly less stable: 80\% showed a negligible log-likelihood difference (<0.02),
and 9\% of runs showed an appreciable log-likelihood difference (>1), with the largest difference being 5.2. However, perhaps suprisingly, 
even for this largest difference results from the default run (e.g. the lfsrs, and the posterior means) were in other ways virtually indistinguishable from the results from the run with the highest log-likelihood \url{github.com/stephens999/ash/dsc-stability/check_convergence.rmd}.
...
These features of the EB approach are extremely attractive in principle.
But their usefulness in practice
depends on reliably estimating the distribution $g$. Estimating $g$ is a ``deconvolution problem",
which is notoriously difficult to solve in general. Indeed, Efron emphasises
...
The key point we want to emphasise here is that a stable non-parametric
approach is not only possible, but in fact quite straightforward, {\it provided we restrict ourselves
...
While the UA is not the only way to achieve this, it is a simple and effective approach.
...
is mostly because the noise level is sufficiently large to make reliable estimation of the multiple modes difficult, rather than reflecting
deficiencies in the specific choices made by mixFdr, and indeed in simulations where the modes are sufficiently well-spaced
to be clearly visible in the observed $\betahat$, mixFdr is able to fit this modes (Supplementary Information; 
\url{dsc-shrink/check_mixfdr_lownoise.rmd}). Of course, we would not advocate the UA in settings where multi-modality is 
clearly visible in the observed $\betahat$.
...
\subsubsection*{Reliable estimates of $g$ lead to reasonable coverage behaviour}
