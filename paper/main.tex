...
Further, this idea has a long history: for example, \cite{tukey:xx}, p32, suggests one should address
\begin{quote}
...
it may be fruitful to apply our methods to the regression context; indeed see \cite{goddard} for work in this vein.) 
...
\end{equation}
so $z_j:=\bhat_j/s_j=\bhat_j$ are standard normal if null $H_j$ holds.
...
as ``half-uniforms", and denote ``ash.hu" in subsequent sections, are here generally similar to ash.u and omitted for clarity.)
...
\subsection*{Differing measurement precision across units}
 ...
 measurement precision across units. The key to this is our use of the likelihood
 (\ref{eqn:lik}), which explicitly incorporates the measurement precision (standard error) of each $\bhat_j$.
...
Example of how shrinkage is adaptive to signal and to standard error?
...
We used the elegant R package {\tt SQUAREM} \cite{squarem}) to accelerate convergence of this EM algorithm.
