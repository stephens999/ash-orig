...
\section{Summary of FDR-related concepts}

Let $\Gamma$ denote a subset of the tests $1,\dots,J$ declared ``significant" by some procedure. 
Define the FDR for $\Gamma$ as the proportion of these tests that are ``false discoveries" in that $H_j$ is true:
\begin{equation}
...
used, considered fixed, and the data $D$, considered random). This expectation may of course depend on $H$ which is unknown, and the goal
of a frequentist procedure is usually to control the $\FDR$ for all possible values of $H$ (``strong control") or specifically when all the $H$ are null (``weak control").

Bayesian approaches, in contrast, condition on the observed data, and compute
