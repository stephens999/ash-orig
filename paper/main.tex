...
Bayes rule from the true $g_1$ and $\pi_0$). As predicted, $\lfdr$ is overestimated, especially in scenarios where 
$\pi_0$ is overestimated. All methods are similarly affected by this, and the ones that most grossly overestimate $\pi_0$ also most grossly overestimated $\lfdr$ and FDR/$q$-values (XXX).
...
\subsection*{Embellishments and Implementation details} \label{sec:implement}
