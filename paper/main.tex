...
We begin by presenting the simplest version of method, and comparing it with existing work, before adding further embellishments.
...
and corresponding (estimated) standard errors $\shat=(\shat_1,\dots,\shat_J)$.  
Initially we assume that the number of observations used to compute these estimates are sufficiently large to justify a normal approximation,
...
We also assume that (given $g$) the $\beta_j$ are independent, and independent of $\shat_j$ (a point we return to later), so
\begin{equation}
...
represent a large and dense grid of fixed positive numbers spanning a range from very small to very big (so $K$ is fixed and large).
 ...
 more flexible alternatives based on using mixtures of uniform distributions, and details (such as choice of gridpoints $\{\sigma_k:k=1,\dots,K\}$) below. 
 ...
 Putting all this together, and making the dependence on hyperparameters $\pi$ explicit, our joint model for $(\beta,\bhat)$ is:
 ...
 This implies that the marginal model $p(\bhat | \shat,\pi)$ is also a mixture of normals:
\begin{equation}
...
\begin{enumerate}
\item Estimate the hyperparameters $\pi$ by maximizing the likelihood $L(\pi)$, yielding $\hat{\pi} := \arg \max L(\pi)$.
...
This approach, which follows  \cite{wakefield:2009},
should be a good approximation if $\bhat_j$ and $\shat_j$ are based on sufficiently many observations.
 




%Given these data, a typical FDR analysis pipeline in genomics would compute $z$ statistics $z_j = \bhat_j/\shat_j$, and a corresponding
$p$ value $p_j$, and then feed these $p$ values to a software package like \qvalue, which will estimate the false discovery rate
...
for moderate numbers of observations, based on replacing this normal likelihood with a $t$ likelihood.
 ...
 t_j | H_j \sim T_\nu
 \end{equation}
 where $T_\nu$ denotes the $t$ distribution on $\nu$ \df, with $\nu$ assumed known. (For simplicity we assume the degrees of freedom are the same for every test,
 but that would be straightforward to relax).
