...
\begin{flushleft}
{\Large
\textbf{Shrinkage, False Discovery Rate, and False Sign Rate Estimation when precision varies across units}
}
...
To illustrate, we provide simulation results for two scenarios, the first being a ``difficult" case where many non-zero $\beta$ are
too close to  zero to be reliably detected, and the second being an ``easier" case where most non-zero $\beta$ are
sufficiently different from zero that they can be reliably detected.

Specifically we use the following distributions for the non-zero $\beta$:
\begin{itemize}
\item[Simulation 1:]
\begin{equation}
...
Emphasize that although pi0 is unidentifiable, the underlying distribution g is estimable
quite accurately.
...
whether or not zero effects exist),  and estimation of lfsr is more robust to modeling assumptions. 
