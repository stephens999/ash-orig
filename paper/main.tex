...
Suppose that we are interested in the values of $J$ ``effects" $\beta_j$ ($j=1,\dots,J$). In some contexts our interest may focus on which of the $\beta_j$ are ``significantly" different from zero, whereas in other contexts our interest may focus on estimating their values; the methods described here are suited to both these contexts.
 We assume that we have obtained data $D_1,\dots,D_J$ that provide independent estimates $\hat\beta_1,\dots,\hat\beta_J$ of these effects,
 with corresponding (estimated) standard errors $s_1,\dots,s_J$. For our purposes, these estimates and standard errors could be obtained from the data
...
A standard genomics analysis pipeline would first convert these numbers to a $Z$ score ($Z_j=\hat\beta_j/s_j$)
and a corresponding $p$ value ($p_j$), testing $H_j:\beta_j=0$.  

Definition of FDR and lfdr (Bayesian); connection to frequentist definitions (Storey).

Then standard methods (e.g. the qvalue package) can be used
...
The second issue is that directly modeling the p values, or z scores, say via non-parametric methods, without taking account of their precision, 
