...
\subsection*{Quantities of interest, and the local false sign rate}

As noted above, the posterior distributions $p(\beta_j | \bhat, \shat, \hat{\pi})$ have a simple analytic form.
In practice it is common, and desirable, to summarize this distribution to convey the ``significance" of observation $j$.

For exampe, previous EB approaches to FDR analysis have focussed on the ``local FDR" 
\cite{efron2008microarrays}
\begin{equation} \label{eqn:fdr}
\fdr_j := \Pr(\beta_j =0  |  \bhat, \shat, \hat{\pi}).
\end{equation}
The ``local" in ``local FDR" refers to the fact that $\fdr_j$ is the probability that the {\it particular}
effect $j$ would be a false discovery, if we were to declare it a discovery.
Storey's $q$ values  \cite{storey.03} provide an alternative way to summarize significance,
and both $\fdr_j$ and a $q$ value for each observation ($q_j$) can be easily computed using our approach (see Appendix \ref{} for details on $q$ value calculations).

Here however, we argue that an alternative quantity, which we refer to as the
``local false sign rate", provides a more practically meaningful measure of significance than either $\fdr_j$ or $q_j$.
